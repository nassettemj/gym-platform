...
export default async function MembersPage({
  ...
  if (memberSummaries.length > 0 && reportStart) {
    ...
    memberSummaries = memberSummaries.map((s) => {
      const rank = rankByMemberId.get(s.member.id) ?? { belt: null, stripes: null, memberType: null, createdAt: null, lastBeltChange: null };
      return {
        ...s,
        belt: rank.belt,
        stripes: rank.stripes,
        memberType: rank.memberType,
        memberSince: rank.createdAt ?? null,
      ...
      {reportStart && (
        <section className="border border-white/10 rounded-xl p-4 space-y-3">
          <ReportTableSection
            ...
            columnLabel="Classes"
            onlyAttended={false}
            availableColumns={MEMBERS_PAGE_COLUMNS}
            rowClickNavigatesToProfile
          />
        </section>
      )}
    </div>
